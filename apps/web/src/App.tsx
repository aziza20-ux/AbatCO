...
import { ArrowRight, Check, CircleCheck, Clock3, FileCheck2, FileText, HandshakeIcon, Menu, Phone, ScanLine, Search, ShieldCheck, X } from 'lucide-react'
import { useTranslation, type Lang } from './i18n/useTranslation'
import type { TranslationKey } from './i18n/en'

type Screen = 'home' | 'services' | 'how-it-works' | 'about' | 'contact'

const screenIds: Screen[] = ['home', 'services', 'how-it-works', 'about', 'contact']
const navKeys: TranslationKey[] = ['nav.home', 'nav.services', 'nav.howItWorks', 'nav.about', 'nav.contact']
...
export default function App() {
  ...
  return <div className="public-site">
    <header className="site-header">
      <button className="site-logo" onClick={() => navigate('home')} aria-label={t('header.logoLabel')}><img src="/icons/icon-192.png" alt="AbatCO" className="logo-icon" /><span>Abat<span>CO</span></span></button>
    ...
    {screen === 'services' && <Services onNavigate={navigate} t={t} />}
...
function Home({ onNavigate, t }: { onNavigate: (screen: Screen) => void; t: T }) {
  return <>
    <main>
      ...
      <section className="section process-section"><div className="section-heading"><p className="eyebrow">{t('home.services.eyebrow')}</p><h2>{t('home.services.heading')}</h2><button className="text-link" onClick={() => onNavigate('services')}>{t('home.process.link')} <ArrowRight size={14} /></button></div><div className="process-grid services-grid"><ServiceCard icon={<HandshakeIcon size={20} />} title={t('home.services.card1.title')} copy={t('home.services.card1.copy')} /><ServiceCard icon={<FileText size={20} />} title={t('home.services.card2.title')} copy={t('home.services.card2.copy')} /><ServiceCard icon={<ScanLine size={20} />} title={t('home.services.card3.title')} copy={t('home.services.card3.copy')} /><ServiceCard icon={<Search size={20} />} title={t('home.services.card4.title')} copy={t('home.services.card4.copy')} /><ServiceCard icon={<ShieldCheck size={20} />} title={t('home.services.card5.title')} copy={t('home.services.card5.copy')} /></div></section>
...
function ServiceCard({ icon, title, copy }: { icon: ReactNode; title: string; copy: string }) {
  return <article className="process-card service-card"><div className="service-card-icon"><span>{icon}</span></div><div><h3>{title}</h3><p>{copy}</p></div></article>
}
...
function Services({ onNavigate, t }: { onNavigate: (screen: Screen) => void; t: T }) {
  return <main className="inner-page"><section className="page-intro"><p className="eyebrow">{t('home.services.eyebrow')}</p><h1>{t('home.services.heading')}</h1><p>{t('home.hero.text')}</p><button className="solid-button" onClick={() => onNavigate('contact')}>{t('header.getStarted')} <ArrowRight size={15} /></button></section><section className="steps-section"><div className="section-heading centered"><p className="eyebrow">{t('home.services.eyebrow')}</p><h2>{t('home.services.heading')}</h2></div><div className="principle-grid"><Principle icon={<HandshakeIcon />} title={t('home.services.card1.title')} copy={t('home.services.card1.copy')} /><Principle icon={<FileText />} title={t('home.services.card2.title')} copy={t('home.services.card2.copy')} /><Principle icon={<ScanLine />} title={t('home.services.card3.title')} copy={t('home.services.card3.copy')} /><Principle icon={<Search />} title={t('home.services.card4.title')} copy={t('home.services.card4.copy')} /><Principle icon={<ShieldCheck />} title={t('home.services.card5.title')} copy={t('home.services.card5.copy')} /></div></section></main>
}
...
function HowItWorks({ onNavigate, t }: { onNavigate: (screen: Screen) => void; t: T }) {
  const steps: [TranslationKey, TranslationKey][] = [
    ...
    ['hiw.step5.title', 'hiw.step5.copy'],
  ]
...
function AboutTrust({ onNavigate, t }: { onNavigate: (screen: Screen) => void; t: T }) { return <main className="inner-page about-page"><section className="about-hero"><div><p className="eyebrow">{t('about.eyebrow')}</p><h1>{t('about.heading1')}<br /><span>{t('about.heading2')}</span></h1><p>{t('about.copy')}</p></div><div className="about-art"><CircleCheck size={44} /></div></section><section className="trust-principles"><div className="section-heading centered"><p className="eyebrow">{t('about.principles.eyebrow')}</p><h2>{t('about.principles.heading')}</h2></div><div className="principle-grid"><Principle icon={<ShieldCheck />} title={t('about.p1.title')} copy={t('about.p1.copy')} /><Principle icon={<FileCheck2 />} title={t('about.p2.title')} copy={t('about.p2.copy')} /><Principle icon={<CircleCheck />} title={t('about.p3.title')} copy={t('about.p3.copy')} /><Principle icon={<ScanLine />} title={t('about.p4.title')} copy={t('about.p4.copy')} /><Principle icon={<HandshakeIcon />} title={t('about.p5.title')} copy={t('about.p5.copy')} /></div></section><section className="about-callout"><p className="eyebrow">{t('about.callout.eyebrow')}</p><h2>{t('about.callout.heading')}</h2><p>{t('about.callout.copy')}</p><button className="outline-button" onClick={() => onNavigate('contact')}>{t('about.callout.button')}</button></section></main> }
...
function Contact({ t }: { t: T }) { return <main className="inner-page contact-page"><section className="contact-intro"><p className="eyebrow">{t('contact.eyebrow')}</p><h1>{t('contact.heading1')}<br /><span>{t('contact.heading2')}</span></h1><p>{t('contact.copy')}</p></section><section className="contact-grid"><aside><p className="eyebrow">{t('contact.ops.eyebrow')}</p><div className="contact-detail"><strong>{t('contact.director.label')}</strong><span>{t('contact.director.value')}</span></div><div className="contact-detail"><strong>{t('contact.phone1.label')}</strong><a href="tel:+250782127438" className="contact-link"><Phone size={11} /> {t('contact.phone1.value')}</a></div><div className="contact-detail"><strong>{t('contact.phone2.label')}</strong><a href="tel:+250784361450" className="contact-link"><Phone size={11} /> {t('contact.phone2.value')}</a></div><div className="contact-detail"><strong>{t('contact.office.label')}</strong><span>{t('contact.office.value')}</span></div><div className="contact-detail"><strong>{t('contact.nationwide.label')}</strong><span>{t('contact.nationwide.value')}</span></div></aside></section></main> }

function Footer({ onNavigate, t }: { onNavigate: (screen: Screen) => void; t: T }) {
  const tagline = t('footer.tagline').split('\n')
  return <footer className="site-footer"><div><button className="site-logo footer-logo" onClick={() => onNavigate('home')}><img src="/icons/icon-192.png" alt="AbatCO" className="logo-icon" /><span>Abat<span>CO</span></span></button><p>{tagline[0]}<br />{tagline[1]}</p></div><div><strong>{t('footer.explore')}</strong><button onClick={() => onNavigate('services')}>{t('nav.services')}</button><button onClick={() => onNavigate('how-it-works')}>{t('footer.howItWorks')}</button><button onClick={() => onNavigate('about')}>{t('footer.about')}</button></div><div><strong>{t('footer.connect')}</strong><button onClick={() => onNavigate('contact')}>{t('footer.contact')}</button><button onClick={() => onNavigate('contact')}>{t('footer.getStarted')}</button></div><small className="copyright">{t('footer.copyright')}</small></footer>
}
